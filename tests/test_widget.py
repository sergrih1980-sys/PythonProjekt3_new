...
def test_mask_account_card_invalid(input_data, error_msg):
...
def test_mask_account_card_other_card_types(input_data, expected_prefix):
...
def test_get_date_invalid(date_string, error_msg):
    """Тестируем некорректные/граничные входные данные."""
    result = get_date(date_string)
    assert result == error_msg
...
def test_get_date_wrong_separators(date_string, expected):
